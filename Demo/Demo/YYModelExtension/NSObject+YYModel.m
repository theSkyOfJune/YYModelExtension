...
+ (instancetype)metaWithClassInfo:(YYClassInfo *)classInfo propertyInfo:(YYClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    if (generic) // YYShadow class
        meta->_hasCustomClassFromDictionary = [generic respondsToSelector:@selector(modelCustomClassForDictionary:)];
    else if (meta->_cls && meta->_nsType == YYEncodingTypeNSUnknown) // 自定义类
    ...
    if ([cls respondsToSelector:@selector(modelCustomPropertyMapper)]) {
        ...
        [customMapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *mappedToKey, BOOL *stop) {
            ...
            if ([mappedToKey isKindOfClass:[NSString class]]) {
                ...
                //next指针将多个属性名映射同一个json key连接起来
                /*
                 2. 多个property对应一个json key
                 @{ @"name"     : @"name", //name.next = null, mapper[name] = name对应的propertyMeta
                    @"fullName" : @"name", // fullName.next = mapper[name]
                    @"username" : @"name" }; // username.next = mapper[name]
                 这样用了一个链表把所有映射到同一个json key的属性串联起来
                 */
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    if (modelMeta->_keyMappedCount >= CFDictionaryGetCount((CFDictionaryRef)dic)) {
        // 字典有多少对Key - Value，它就会调用多少次这个函数
        // CFArrayApplyFunction() 和 CFDictionaryApplyFunction() 方法来遍历容器类能带来不少性能提升
        // CFDictionaryApplyFunction((CFDictionaryRef)dic, ModelSetWithDictionaryFunction, &context);
        CFDictionaryApplyFunction((CFDictionaryRef)dic, ModelSetWithDictionaryFunction, &context);
//        CFDictionaryApplyFunction((CFDictionaryRef)modelMeta->_mapper, ModelSetWithDictionaryFunction, &context);
...
+ (NSArray *)modelArrayWithKeyValuesArray:(id)json {
...
+ (NSArray *)modelArrayWithFilename:(NSString *)filename {
    ...
    return [self modelArrayWithFile:[[NSBundle mainBundle] pathForResource:filename ofType:nil]];
}

+ (NSArray *)modelArrayWithFile:(NSString *)file {
    if (!file) return nil;
    return [self modelArrayWithKeyValuesArray:[NSArray arrayWithContentsOfFile:file]];
}
